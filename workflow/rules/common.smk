...
units = pd.read_csv(config["UNITS"], sep="\t", comment="#").set_index(["sample", "unit"], drop=False)
...
def get_technical_replicates(wildcards):
    ...
    return expand(OUTPUT_DIR + f"/aligned_{libtype}/{wildcards.sample}_{{trep}}.bam", trep = treps)
